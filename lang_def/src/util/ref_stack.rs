...
impl<Item, ExtraData: Copy> RefStack<Item, ExtraData> {
    ...
    /// Like `with_items`, but accepts an iterator instead of a slice. Uses significantly more
    /// stack space.
    pub fn with_item_iter<R>(
        &self,
        ...
        if let Some(item) = iter.next() {
            self.with_item(&item, |new_stack| new_stack.with_item_iter(iter, f))
        } else {
            f(self)
        }
    }

    /// Like `with_items`, but accepts an iterator return references instead of a slice. Uses
    /// significantly more stack space.
    pub fn with_item_ref_iter<'a, R>(
        &self,
        ...
        Item: 'a,
    {
        let mut iter = iter.into_iter();
        if let Some(item) = iter.next() {
            self.with_item(item, |new_stack| new_stack.with_item_ref_iter(iter, f))
        } else {
            f(self)
        }
    }

    /// Similar to `with_item_ref_iter`, but accepts an iterator that returns slices.
    pub fn with_items_ref_iter<'a, R>(
        &self,
        iter: impl IntoIterator<Item = &'a [Item]>,
        f: impl FnOnce(&Self) -> R,
    ) -> R
    ...
    {
        let mut iter = iter.into_iter();
        if let Some(items) = iter.next() {
            self.with_items(items, |new_stack| new_stack.with_items_ref_iter(iter, f))
        } else {
            f(self)
        }
    }
